...
class CreateFlightVehicle:
    """
    Factory method for generating an aerosandbox `airplane` object.
    ...
    def __init__(
        ...
        """
        Initialize the Aero class.

        Parameters
        ----------
        aero_geom : object, optional
            The aero_geom object representing the geometry of the aircraft.
        nacelles_enabled : bool, optional
            Whether or not the nacelles are enabled. Defaults to False.
        generate_airfoil_polars : bool, optional
            Whether or not to generate airfoil polars. Defaults to True.

        Attributes
        ----------
        ag : object
            The aero_geom object representing the geometry of the aircraft.
        nacelles_enabled : bool
            Whether or not the nacelles are enabled.
        generate_airfoil_polars : bool
            Whether or not to generate airfoil polars.
        airfoils_generated : bool
            Whether or not the airfoils have been generated.
        """
        self.ag = aero_geom
...
class CreateAirplane(CreateFlightVehicle):
    ...
    Methods
    -------
    __generate_airfoil(airfoil_name: str)
        Generates the polars for each of the airfoils using Xfoil.

    __generate_fuselage()
        Generates the fuselage sections from the input geometry.

    __generate_nacelles()
        Generates the nacelles from the input geometry.

    __generate_wings()
        Generates the wing and control surface sections from the input geometry.

    __calculate_MAC()
        Calculates the mean aerodynamic chord and aerodynamic center of the airplane wings.

    generate_all_airfoils()
        Generates all the airfoils for the wings, vertical and horizontal stabilizers, and canard.

    generate()
        Creates an AeroSandBox airplane object from the input geometry.

    update(latest_geom=None)
        Updates the airplane's wings and control surfaces.

    output()
        Generates and returns the airplane object.
    """
    ...
    def __calculate_MAC(self):
        """
        Calculate the mean aerodynamic chord (MAC) and aerodynamic center of the first airplane wing.
    ...
    def generate_all_airfoils(self):
        ...
        """
        Generate airfoils for each section of the aircraft.
        ...
        Notes
        -----
        This function generates airfoils for the wing, vertical tail, horizontal tail, and canard sections of the aircraft.
        The airfoils are stored in the respective attributes of the class instance. After calling this function,
        the `airfoils_generated` attribute will be set to `True`.
        """
        self.wing_airfoils = [
    ...
    def output(self):
        """
        Generate and return the airplane.

        Returns
        -------
        airplane : object
            The generated airplane object.
        """
        self.generate()
